...
from hawkeye.agent import LlmConfig, probe_llm, write_capability_diagnostics
...
def build_parser() -> argparse.ArgumentParser:
    ...
    app_parser = subcommands.add_parser(
        "app",
        help="Run the complete local investigator application with persistent local data",
    )
    app_parser.add_argument(
        "--data",
        type=Path,
        default=Path("data"),
        help="Local application data root containing cases, workspace, and comparisons",
    )
    app_parser.add_argument(
        "--port", type=int, default=8760, help="Local loopback port (1024-65535)"
    )
    ...
    probe_parser = subcommands.add_parser(
        "llm-probe",
        help="Opt in to one strict, secret-free OpenAI-compatible provider handshake",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "llm-probe":
        return _run_llm_probe(args)
...
def _run_app(args: argparse.Namespace) -> int:
    """Bootstrap one explicit local data root and launch the complete application."""
    ...
    try:
        for directory in (cases_root, workspace_root, comparisons_root):
    ...
    except (CaseIntegrityError, OSError, ValueError) as error:
        print(json.dumps({"status": "rejected", "error": str(error)[:500]}, indent=2))
        return 2
    return 0
...
def _run_llm_probe(args: argparse.Namespace) -> int:
    """Persist an explicit opt-in handshake without ever serializing credentials."""

    try:
        config = LlmConfig.from_environment()
        if config is None:
            raise ValueError(
                "No model provider is configured; deterministic fallback remains active"
            )
        diagnostics = probe_llm(config)
