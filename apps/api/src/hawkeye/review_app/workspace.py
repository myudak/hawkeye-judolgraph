...
from hawkeye.agent import LlmConfig, ModelInvestigator, OpenAICompatibleClient
...
class MvpWorkspace:
    def __init__(
        ...
        *,
        ...
        self._capability_status: dict[str, object] | None = None
        self._investigator: ModelInvestigator | None = None

    def capability_status(self) -> dict[str, object]:
        """Report configuration without an automatic or billable provider request."""

        if self._capability_status is None:
            try:
                ...
                self._capability_status = {
                    ...
                    "selected_model": None,
                    "safe_to_enable_model_path": False,
                }
            else:
                if config is None:
                    self._capability_status = {
                    ...
                    self._investigator = ModelInvestigator(OpenAICompatibleClient(config))
                    self._capability_status = {
