...
SOURCE_DIR = ROOT / "competition" / "gemastik-2026"
...
DELIVERABLES = (
    Deliverable(
        source=SOURCE_DIR / "technical" / "TECHNICAL_DOCUMENT.md",
    ...
    Deliverable(
        source=SOURCE_DIR / "technical" / "LIBRARIES_AND_LICENSES.md",
    ...
    Deliverable(
        source=SOURCE_DIR / "technical" / "LICENSE_ADOPTION.md",
...
def add_image(document: Document, source: Path, relative_path: str, alt_text: str) -> None:
...
def add_markdown_content(document: Document, source: Path, skip_headings: int, toc: bool) -> None:
    ...
    while index < len(lines):
        ...
        if image_match:
            add_image(document, source, image_match.group(2), image_match.group(1))
            index += 1
            continue
